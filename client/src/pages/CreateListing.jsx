...
const CreateListing = () => {
  ...
  const getDownloadUrl = async (fileKey) => {
    try {
      const result = await axios.get(
        `http://localhost:3000/api/images/Url/${fileKey}`
      );
  ...
  const UploadImages = async () => {
    ...
    if (files.length === 0) {
      setImageUploadError("You must select at least one image to upload.");
      setUploading(false);
      return;
    }

    if (files.length + formData.imageUrl.length > 6) {
    ...
    try {
      const keyPromises = files.map((file) => storeImage(file));
      const keys = await Promise.all(keyPromises);
      const updatedKeys = formData.imageKeys.concat(keys);

      const urlPromises = updatedKeys.map((key) => getDownloadUrl(key));
      const urls = await Promise.all(urlPromises);
      const updatedUrls = formData.imageUrl.concat(urls);

      setFormData((prevFormData) => ({
        ...prevFormData,
        imageKeys: updatedKeys,
  ...
  const handleRemoveImage = async (index) => {
    try {
      await axios.delete(
        `http://localhost:3000/api/images/delete/${formData.imageKeys[index]}`
      );
  ...
  const handleSubmit = async (e) => {
    ...
    if (formData.imageKeys.length < 1) {
