...
export default function Profile() {
  ...
  const handleDeleteAccount = async () => {
    try {
      ...
      const listingsRes = await fetch(`http://18.185.114.0:3000/api/user/listings/${currentUser._id}`);
      ...
      for (const listing of userListings) {
        ...
        for (const key of imageKeys) {
          await fetch(`http://18.185.114.0:3000/api/images/delete/${key}`, {
            method: 'DELETE',
          });
        }
        await fetch(`http://18.185.114.0:3000/api/listing/delete/${listing._id}`, {
          method: 'DELETE',
        });
      }
      const res = await fetch(`http://18.185.114.0:3000/api/user/delete/${currentUser._id}`, {
        method: 'DELETE',
      });
  ...
  const handleUpdate = async (e) => {
    ...
    try {
      ...
      const res = await fetch(`http://18.185.114.0:3000/api/user/update/${currentUser._id}`, {
        method: 'POST',
        headers: {
  ...
  const handleSignOut = async (e) => {
    ...
    try {
      ...
      const res = await fetch(`http://18.185.114.0:3000/api/auth/signout`);
      const data = await res.json();
      if (data.success === false) {
  ...
  const handleShowListings = async () => {
    try {
      ...
      const res = await fetch(`http://18.185.114.0:3000/api/user/listings/${currentUser._id}`)
      const data = await res.json();
      if (data.success === false) {
        setShowListingsError(true);
        return;
  ...
  const handleDeleteListing = async (listingId) => {
    try {
      ...
      const res = await fetch(`http://18.185.114.0:3000/api/listing/delete/${listingId}`, {
        method: 'DELETE',
      });
      const data = await res.json();
      if (data.success === false) {
      ...
      await Promise.all(listingToDelete.imageKeys.map(async (imageKey) => {
        await fetch(`http://18.185.114.0:3000/api/images/delete/${imageKey}`, {
          method: 'DELETE'
        });
      }));
